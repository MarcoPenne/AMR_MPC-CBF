...
def export_car_ode_model(path, l1, l2, fixed_obstacles, other_path, other_obstacles, dT, n_lap):
    ...
    #x_obs = MX.sym('x_obs')
    #y_obs = MX.sym('y_obs')
    #theta_obs = MX.sym('theta_obs')
    #v_obs = MX.sym('v_obs')
    #p = vertcat(x_obs, y_obs, theta_obs, v_obs)
    p = vertcat([])
    ...
    y_car_1 = yref_s(x_t_plus_1[0]) + cos(thetar_car_1)*x_t_plus_1[1]
        ...
        #(x - x_obs)**4/(l1)**4 + (y- y_obs)**4/(l1)**4  -5

    #h_t = ((x_car - x_obs)**4/(l1)**4) + ((y_car-y_obs)**4/(l2)**4) - 5
    #h_t_plus_1 = ((x_car_1 - (x_obs+cos(theta_obs)*v_obs*dT) )**4/(l1)**4) + ((y_car_1 - (y_obs+sin(theta_obs)*v_obs*dT))**4/(l1)**4) - 5
